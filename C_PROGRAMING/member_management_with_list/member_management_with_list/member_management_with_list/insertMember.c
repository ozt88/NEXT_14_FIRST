...
int insertMember(list_t* userInfoList)
{
	...
	newMember = makeNode();
	prevNode = findMaxInListWithNode(userInfoList);
	system("cls");
	printf("=============================================================================\n");
	printf("%40s\n", "회원 등록");
	printf("=============================================================================\n");

	printf("\t회원 ID: %d\n", prevNode->data.userId + 1);
	newMember->data.userId = prevNode->data.userId + 1;
...
//ID를 입력하는 함수 여기서는 쓰이지 않지만 나중에 검색에 사용됨
